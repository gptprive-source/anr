...
const MessageBubble = ({
  ...
  const canDeleteForEveryone = isOwn && message.created_at && 
    (Date.now() - new Date(message.created_at).getTime()) < 7 * 60 * 1000;
      ...
      case "missed_call":
        return (
          <div className="flex items-center gap-2 text-destructive">
            ...
            <span className="text-sm">Appel manqué</span>
          </div>
        );
        ...
        return (
          <div className="flex items-center gap-2 text-primary">
            ...
            <span className="text-sm">
              Appel • {message.call_duration_seconds 
                ? `${Math.floor(message.call_duration_seconds / 60)}:${(message.call_duration_seconds % 60).toString().padStart(2, "0")}`
                : "0:00"
              }
            </span>
          </div>
  ...
  return (
    <div className={cn(
      ...
      <div 
        className={cn(
          "relative max-w-[80%] px-3 py-2 rounded-lg",
          isOwn 
      ...
      >
        ...
        {showActions && (
          <div 
            className={cn(
              "absolute top-full mt-1 z-10 bg-popover border border-border rounded-lg shadow-lg py-1 min-w-[140px]",
              isOwn ? "right-0" : "left-0"
            )}
          >
            {message.message_type === "text" && message.content && (
              <button
                onClick={(e) => { e.stopPropagation(); onCopy(); setShowActions(false); }}
            ...
            <button
              onClick={(e) => { e.stopPropagation(); onForward(); setShowActions(false); }}
              className="w-full px-3 py-2 text-sm text-left hover:bg-muted flex items-center gap-2"
            >
            ...
            {canDeleteForEveryone && (
              <button
                onClick={(e) => { e.stopPropagation(); onDeleteForEveryone(); setShowActions(false); }}
                className="w-full px-3 py-2 text-sm text-left hover:bg-muted flex items-center gap-2 text-destructive"
              >
...
const Chat = () => {
  ...
  const startVoiceRecording = async () => {
    try {
      ...
      recorder.onstop = async () => {
        ...
        if (chatId) {
          await sendMessage(chatId, { voiceUrl: urlData.publicUrl });
        }
  ...
  const handleSendMedia = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const { data, error } = await supabase.storage
        .from("avatars") // Using existing bucket, could create a dedicated one
  ...
  const displayName = recipient?.first_name && recipient?.last_name
    ...
    : recipient?.first_name || "Utilisateur";
